...
document.addEventListener('mousemove', function (event) {
    ...
    eyeLeft.style.transform = `translate(${Math.cos(angle - 0.3) * distance}px, ${Math.sin(angle - 0.3) * distance}px)`;
    eyeRight.style.transform = `translate(${Math.cos(angle + 0.3) * distance}px, ${Math.sin(angle + 0.3) * distance}px)`;
});
...
robot.addEventListener('click', () => chatContainer.style.transform = 'translateY(0)');
